...
class RestApiConfig(BaseModel):
    ...
    response_behavior: Optional[Dict[Union[int, Literal["default"]], Behavior]] = {
        "default": ReturnToLlmBehavior(llm_response="The resource you are looking for was not found."),
    }
    query_path: Optional[str] = None
    parameter_location: Optional[ParameterLocation] = None
    ...
    def __hash__(self) -> int:
        return hash((
            ...
            self.method,
            self.auth,
            tuple(self.response_behavior.items()) if self.response_behavior is not None else None,
            self.query_path,
            self.parameter_location
        ))
    
    def __eq__(self, other: Any) -> bool:
        return isinstance(other, RestApiConfig) and \
            self.url == other.url and \
            self.method == other.method and \
            self.auth == other.auth and \
            self.response_behavior == other.response_behavior and \
            self.query_path == other.query_path and \
            self.parameter_location == other.parameter_location
